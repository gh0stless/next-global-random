...
- **Jeder Sideload (Datei-Upload + Versions-Bump + disable/enable-Zyklus) läuft ab jetzt zwischen `occ maintenance:mode --on` und `occ maintenance:mode --off`** — deckt genau das Zeitfenster ab, in dem die Datei-Version und die DB-Version kurzzeitig auseinanderlaufen (siehe Punkt darüber) und andere Nutzer sonst live auf die "Aktualisierung erforderlich"-Wall laufen könnten. Als Standardvorgehen festgelegt am 08.08.2026, nachdem genau das zweimal live passiert ist.
